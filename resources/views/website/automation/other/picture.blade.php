    ...
    <textarea id="myInput" style="opacity:0">
      <div id="picture-Section:{{ $img_name }}">
        ...
        <div class="relative amp-shadow bordering" amp-fx="fade-in-scroll" data-repeat>
          ...
          srcset="{{ $b }}{ asset('assets/blog/alz/{{ $img_folder }}/{{ $img_name }}_1280.webp') }{{ $a }},
          {{ $b }}{ asset('assets/blog/alz/{{ $img_folder }}/{{ $img_name }}_2560.webp') }{{ $a }} 2x"
          src="{{ $b }}{ asset('assets/blog/alz/{{ $img_folder }}/{{ $img_name }}_2560.webp') }{{ $a }}" height="1440" width="2560">
          <amp-img fallback alt="{{ $desc }}" layout="responsive" class="bordering"
          srcset="{{ $b }}{ asset('assets/blog/alz/{{ $img_folder }}/{{ $img_name }}_1280.jpg') }{{ $a }},
          {{ $b }}{ asset('assets/blog/alz/{{ $img_folder }}/{{ $img_name }}_2560.jpg') }{{ $a }} 2x"
          src="{{ $b }}{ asset('assets/blog/alz/{{ $img_folder }}/{{ $img_name }}_2560.jpg') }{{ $a }}" height="1440" width="2560">
          </amp-img></amp-img>
      ...
      <amp-lightbox id="piclgtbx{{ $tap }}" layout="nodisplay">
        <div class="dark_ovrlay_lightroom" on="tap:piclgtbx{{ $tap }}.close" role="button" tabindex="0"></div>
            <div class="popup">
              ...
              <a class="iconsocialdownload2" href="{{ $b }}{ asset('assets/blog/alz/{{ $img_folder }}/{{ $img_name }}_2560.jpg') }{{ $a }}" download title="Download {{ $desc }}">
              ...
              <amp-social-share class="rounded"
                type="pinterest"
                data-param-media="{{ $b }}{ asset('assets/blog/alz/{{ $img_folder }}/{{ $img_name }}_2560.jpg') }{{ $a }}"
                width="55"
                height="55"></amp-social-share>
              <amp-social-share class="rounded"
                type="tumblr"
              ...
              <amp-social-share class="rounded"
                ...
                <div class="relative copybox">
                  <p class="sans_serif_p" style="margin:0; padding:0;">{{ $b }}{ asset('assets/blog/alz/{{ $img_folder }}/{{ $img_name }}_2560.jpg') }{{ $a }}</p>
                </div>
